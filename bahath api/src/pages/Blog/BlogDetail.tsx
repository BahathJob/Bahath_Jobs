...
export function BlogDetail() {
  ...
  const fetchPost = async () => {
    try {
      ...
      // Fetch related posts
      const relatedResponse = await api.get(`/blog?category=${response.data.category}&limit=3`);
      setRelatedPosts(relatedResponse.data.posts.filter((p: any) => p.slug !== slug).slice(0, 2));
    } catch (error) {
              ...
              <Card>
                ...
                <div className="space-y-4">
                  {relatedPosts.map((relatedPost: any) => (
                    <Link key={relatedPost.id} to={`/blog/${relatedPost.slug}`} className="block group">
                      <div className="flex space-x-3">
                        <img
                          src={relatedPost.featured_image || getDefaultImage(relatedPost.category)}
                  ...
                  {relatedPosts.length === 0 && (
                    <div className="flex space-x-3">
                      <img
                        src="https://images.pexels.com/photos/4226140/pexels-photo-4226140.jpeg?auto=compress&cs=tinysrgb&w=200"
                        alt="Remote Work"
                        className="w-16 h-16 object-cover rounded-lg"
                      />
                      <div className="flex-1">
